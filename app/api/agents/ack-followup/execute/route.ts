...
import { executeAckFollowUp } from '@/src/services/data/csvDataService';
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const id of recommendationIds) {
      const success = await executeAckFollowUp(id);
      results.push({ id, success });
    }

    return Response.json({ success: true, results });
  } catch (e: any) {
    console.error('Failed to execute ack follow-ups:', e);
